...
mkdir -p "$PORT_ROOT"
: > "$PASSTHROUGH_LIST"
: > "$REPORT_DIR/excluded_source_only.txt"
    ...
    case "$sel" in
        source)
            ...
            if [ -d "$src_tree" ]; then
                ...
                # -HAX + root: keep hardlinks, ACLs, SELinux labels and
                # file capabilities that 05 preserved at extraction.
                sudo rsync -aHAX --delete "$src_tree/" "$PORT_ROOT/$name/"
            else
        ...
        SOURCE-ONLY-EXCLUDED)
            # Source-device-only partition: decided in 04, never packaged.
            echo "$name" >> "$REPORT_DIR/excluded_source_only.txt"
            ;;
        target)
